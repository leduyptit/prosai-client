...
interface PropertyRankingProps {
  activeRankingTab: number;
  rankingLoading: boolean;
  rankingData: PropertyRankingItem[][];
  onRankingTabChange: (index: number) => void;
  onLoadMore: () => void;
  hasMoreData: boolean;
  loadingMore: boolean;
}

const PropertyRanking: React.FC<PropertyRankingProps> = ({
  activeRankingTab,
  rankingLoading,
  rankingData,
  onRankingTabChange,
          ...
          <div className="flex items-center">
            {rankingTabs.map((tab, index) => (
              <div key={index} className="py-4">
                <button
                  onClick={() => onRankingTabChange(index)}
